...
public class MySqlJdbcContext implements AutoCloseable {
    ...
    public boolean isGtidModeEnabled() {
        ...
        try {
            jdbc().query("select @@global.gtid_current_pos", rs -> {
                if (rs.next()) {
                    mode.set(rs.getString(1)==null?"off":"on");
                }
            });
        } catch (SQLException e) {
    ...
    public String knownGtidSet() {
        AtomicReference<String> gtidSetStr = new AtomicReference<String>();
        try {
            jdbc.query("SELECT @@global.gtid_binlog_pos", rs -> {
    ...
    public String purgedGtidSet() {
        AtomicReference<String> gtidSetStr = new AtomicReference<String>();
        try {
        	// rs.getString(1) => binlogFilename
        	// rs.getInt(5)    => pos
            jdbc.query("show binlog events limit 1", rs -> {
                if (rs.next() && rs.getMetaData().getColumnCount() > 0) {
                	jdbc.query(String.format("select binlog_gtid_pos('%s', %d)", rs.getString(1), rs.getInt(5))
                			, subrs -> {
                        if (subrs.next() && subrs.getMetaData().getColumnCount() > 0) {
                        	gtidSetStr.set(subrs.getString(1));
                        }
                    });                
                }
            });
